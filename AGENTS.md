...
| Mappings | **Mojang official (mojmap)** | Class names are `ResourceLocation`, `Item`, `Level`, `Player`. **Never write Yarn names** (`Identifier`, `World`, `PlayerEntity`) — Yarn does not exist for 26.x. |
...
1. **Never invent an API.** If you are not certain a Minecraft or Fabric method exists in
   `26.2`, do not guess a plausible name. Check the decompiled sources in the IDE, or
   <https://docs.fabricmc.net/develop/> with the version selector set to 26.2.
   A wrong guess costs a full Gradle build to discover.
...
| `ResourceLocation` path | `snake_case`, English | `training_dummy` |
...
`ResourceLocation.fromNamespaceAndPath("digicube", ...)` inline, and never a bare string
...
- **Do not add co-author trailers.** No `Co-Authored-By` lines, and no "generated by"
  or tool-attribution footers. This applies to AI agents committing here too: the
